...
{
    public class UpdateCustomerValidate : AbstractValidator<UpdateCustomerDto>
    {
        public UpdateCustomerValidate()
        {
            RuleFor(x => x.Id).NotEmpty();
            RuleFor(x => x.FirstName).NotEmpty().WithMessage("Lütfen Adınızı Yazınız").MaximumLength(15)
                .WithMessage("İsminiz En fazla 15 karakterden oluşabilir");
            RuleFor(x => x.LastName).NotEmpty().WithMessage("Lütfen Soyadınızı Yazınız").MaximumLength(15)
                .WithMessage("Soyadınız En fazla 15 karakterden oluşabilir");
            RuleFor(x => x.Mail).NotEmpty().WithMessage("Lütfen Adınızı Yazınız").EmailAddress()
